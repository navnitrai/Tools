...
public class Main extends Thread {
  ...
  public void run() {
    ...
    String lowerCase = prop.getProperty("keyType").trim().toLowerCase();
    
    if(lowerCase.equalsIgnoreCase("usbtoken"))
    {
    ...
    while (true) {
      ...
      if (!found && Main.count.get() == 0 && tpe.getTaskCount() == tpe.getCompletedTaskCount() && workQueue.size() == 0) {
        ...
        if (inputDirectory.toFile().exists()) {
          try
          {
        	  DirectoryStream<Path> stream = Files.newDirectoryStream(inputDirectory);
            ...
            for (Path inFile : stream) {
              ...
              {
            	  String[] list = inFile.toFile().list();
            	  
            	  if(list != null && list.length > 0)
            	  {
            		  for (int i = 0; i < list.length; i++) 
            		  {
            			  Path newFIlePath = Paths.get(inFile.toFile().getAbsolutePath(),list[i]);
            			  String subFolderName = inFile.toFile().getName();
            			  tpe.execute(new DecryptEIDUID(newFIlePath , subFolderName));
            		  }
            	  }
              }
              {
            	  if(inFile.toFile().isFile())
            		  tpe.execute(new DecryptEIDUID(inFile, null));
              }
            }
          } catch (Exception e) {
            e.printStackTrace();
